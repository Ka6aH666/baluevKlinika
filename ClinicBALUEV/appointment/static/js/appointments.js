...
let service_celected_id = $('#service_celected_id').val() || null;
...
const calendar = new FullCalendar.Calendar(calendarEl, {
    ...
    dateClick: function (info) {
        ...
        getAvailableSlots(info.dateStr, staff_memeber_id, service_celected_id);
    },
...
$(document).ready(function () {
    ...
    getAvailableSlots(currentDate, staff_memeber_id, service_celected_id);
});
...
body.on('click', '.djangoAppt_btn-request-next-slot', function () {
    const serviceId = $(this).data('service_celected_id');
...
body.on('click', '.btn-submit-appointment', function () {
    ...
    if (selectedSlot && selectedDate) {
        ...
        fetchServiceDuration(service_celected_id, function(serviceDuration) {
            if (serviceDuration !== null) {
                ...
                form.append($('<input>', {type: 'hidden', name: 'staff_member', value: staff_memeber_id}));

                form.submit();

            } else {
                console.warn('Не удалось получить длительность услуги.');
            }
        });
    } else {
...
$('#service_celected_id').on('change', function () {
    service_celected_id = $(this).val() || null;
    let currentDate = null
    if (selectedDate == null) {
        currentDate = new Date().toLocaleDateString('en-CA');
    } else {
    ...
    fetchCalenderSettingsForStaff(staff_memeber_id);
    fetchNonWorkingDays(staff_memeber_id, function (newNonWorkingDays) {
        nonWorkingDays = newNonWorkingDays;  // Update the nonWorkingDays array
        calendar.render();  // Re-render the calendar to apply changes
...
function fetchNonWorkingDays(staff_memeber_id, callback) {
    if (!staff_memeber_id || staff_memeber_id === 'none') {
...
function fetchCalenderSettingsForStaff(staff_memeber_id) {
    if (!staff_memeber_id || staff_memeber_id === 'none') {
        callback([]);
        return;
    }
...
function fetchServiceDuration(serviceId, callback) {
    ...
    $.ajax({
        url: fetch_service_duration,
        data: ajaxData,
        dataType: 'json',
        success: function (data) {
            if (data.error) {
                console.error('Ошибка при получении длительности услуги:', data.message);
                callback(null);
            } else {
...
function getAvailableSlots(selectedDate, staff_memeber_id = null, service_celected_id = null) {
...
//    }
    ...
    let ajaxData = {
        ...
        'staff_member': staff_memeber_id,
        'service': service_celected_id,
    };
    fetchNonWorkingDays(staff_memeber_id, function (nonWorkingDays) {
            ...
            if (data.available_slots.length === 0) {
                ...
                if (selectedD < today) {
                    ...
                    if (slotContainer.find('.djangoAppt_btn-request-next-slot').length === 0) {
                        slotContainer.append(`<button class="btn btn-danger djangoAppt_btn-request-next-slot" data-service-id="${service_celected_id}">` + requestNonAvailableSlotBtnTxt + `</button>`);
                    }
