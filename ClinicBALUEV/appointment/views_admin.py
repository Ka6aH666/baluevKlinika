...
from .forms import (CalendarSettingsForm, PersonalInformationForm, ServiceForm,
                    StaffAppointmentInformationForm, StaffMemberForm, Spicializationform)
from .models import (Appointment, ArchivedAppointment, DayOff, Service,
                     StaffMember, WorkingHours, Specialization)
...
def add_or_update_service(request, service_id=None):
    if service_id:
        ...
        page_title = _("Просмотр сервиса")
        btn_text = _("Сохранить")
    else:
        service = None
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            if service:
                messages.success(request, _("Служба успешно сохранена"))
            else:
                messages.success(request,
                                 _("Услуга успешно добавлена. Не забудьте выбрать врача, предоставляющего услугу.e"))
...
def delete_service(request, service_id):
    ...
    try:
        service.delete()
        messages.success(request, _("Служба удалена"))
        return redirect("get_service_list")
    except ProtectedError:
        messages.info(request, _("Чтобы удалить службу, вы должны вручную удалить все записи для этой службы"
                                 " и уберите это из предлагаемых врачем услуг"))
    return redirect("get_service_list")
...
def add_staff_member_info(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect("add_staff_member_info")
    else:
...
def update_personal_info(request, staff_user_id=None, response_type="html"):
    ...
    if request.method == "POST":
        ...
        if is_valid:
            if staff_user_id:
                return redirect("user_profile", staff_user_id)
...
def update_day_off(request, day_off_id, staff_user_id=None, response_type="html"):
    ...
    if not check_extensive_permissions(request.user.id, request.user, day_off):
        message = _("Вы можете обновить только свои собственные выходные дни.")
        return handle_unauthorized_response(request, message, response_type)
...
def delete_day_off(request, day_off_id, response_type="html"):
    ...
    if not check_extensive_permissions(request.user.id, request.user, day_off):
        message = _("Вы можете удалить только свои собственные выходные дни.")
        return handle_unauthorized_response(request, message, response_type)
    day_off.delete()
    messages.success(request, "")
...
def update_working_hours(
    ...
    if not check_extensive_permissions(request.user.id, request.user, working_hours):
        message = _("Вы можете обновлять только свои собственные рабочие часы.")
        return handle_unauthorized_response(request, message, response_type)
    staff_user_id = staff_user_id or request.user.pk
    ...
    if appointment_id:
        ...
        if request.user.is_superuser:
            ...
            if not Appointment.objects.filter(id=appointment_id,
                ...
                return json_response(_("У вас нет разрешения на доступ к этой записи."), status_code=403)
        ...
        try:
            ...
            if not request.user.is_superuser:
                return json_response(_("Вы не являетесь сотрудником. Не удается выполнить это действие!"), status=400,
                                     success=False)
            else:
                services = list(Service.objects.all().values('id', 'name'))

    if len(services) == 0:
        return json_response(_("Этот сотрудник не предлагает никаких услуг."), status=404, success=False,
                             error_code=ErrorCode.SERVICE_NOT_FOUND)
    return json_response(_("Успешно выбранные службы."), custom_data={'services_offered': services})
...
def update_appt_date_time(request):
    ...
    try:
        ...
        return json_response(_("Записи на прием не существует."), status=404, success=False,
                             error_code=ErrorCode.APPOINTMENT_NOT_FOUND)
    except ValidationError as e:
        return json_response(e.message, status=400, success=False)
    return json_response(_("Запись на прием успешно обновлена."), custom_data={'appt': appt.id})
...
def is_user_staff_admin(request):
    ...
    try:
        ...
        return json_response(_("Пользователь является сотрудником компании."), custom_data={'is_staff_admin': True})
    except StaffMember.DoesNotExist:
        # if superuser, all rights are granted even if not a staff member
        if not user.is_superuser:
            return json_response(_("Пользователь не является штатным сотрудником."), custom_data={'is_staff_admin': False})
        return json_response(_("Пользователь является суперпользователем."), custom_data={'is_staff_admin': True})
...
@require_user_authenticated
@require_staff_or_superuser
def edit_calendar_settings(request, staff_member_user_id):
    print(staff_member_user_id, "staff_member_user_idstaff_member_user_idstaff_member_user_idstaff_member_user_idstaff_member_user_idstaff_member_user_idstaff_member_user_idstaff_member_user_idstaff_member_user_idstaff_member_user_id")
    staff_member, settings = check_exists_calander_settings(request, staff_member_user_id)

    if request.method == "POST":
        form = CalendarSettingsForm(request.POST, instance=settings)
        if form.is_valid():
            form.save()
            return redirect("user_profile", staff_member_user_id)
    else:
    ...
    return render(request, "administration/calendar_settings_form.html", {"form": form, "start_date": start_date,
                                                                          "end_date": end_date})
def add_specialization(request, spec_id=None):
    if spec_id:
        label = "Редактирование специализацию"
        btn = "Сохранить"
        instance = get_object_or_404(Specialization, id=spec_id)
    else:
        label = "Добавление специализацию"
        btn = "Добавить"
        instance = None
    if request.method == "POST":
        form = Spicializationform(request.POST, instance=instance)
        if form.is_valid():
            form.save()
            return redirect("get_specialization_list")
    else:
        form = Spicializationform(instance=instance)
    return render(request, "administration/manage_specialization.html", {"form": form, "btn": btn, "label": label})

def get_specialization_list(request):
    spec_list = Specialization.objects.all()
    return render(request, "administration/speclist.html", {"spec_list": spec_list})