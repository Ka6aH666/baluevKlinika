...
def fetch_service_duration(request):
    ...
    try:
        ...
        return JsonResponse({'error': True, 'message': 'Услуга не найдена'}, status=404)
...
def get_available_slots_ajax(request):
    ...
    if days_off_exist:
        message = _("Выходной, пожалуйста, выберите другой день.")
...
def get_next_available_date_ajax(request, service_id):
    ...
    if staff_id and staff_id != "none":
        ...
        data = {"error": True}
        message = _("Ни один сотрудник не выбран")
        return json_response(
            message=message,
            custom_data=data,
            success=False,
...
def appointment_request(request, service_id=None, staff_member_id=None):
    ...
    if not request.user.is_authenticated:
        messages.error(request, _("Для записи вы должны быть зарегистрированы"))
...
def appointment_request_submit(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            print(f"👤 Сотрудник: {staff_member}")
            ...
            messages.error(
                request,
                _("В вашей заявке была допущена ошибка. Пожалуйста, проверьте форму и повторите попытку."),
            )
    else:
